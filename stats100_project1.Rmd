...
NBASalaries = read.csv("basketball-2022-salary.csv") #%>% filter(Date > )
#View(NBASalaries)
...
dfResult <- data.frame(coeffs, NBASalaries$X2022.23, NBASalaries$Rk)# %>% mutate(Salaries = NBASalaries$X2022.23 %>% str_remove_all("\\$,") %>% as.numeric())
dfResult$Salaries = as.numeric(gsub("\\$", "", dfResult$NBASalaries.X2022.23))
dfResult$Salaries = dfResult$Salaries / 1000000

#for(i in 1:nrow(dfResult)) {
 # dfResult[formattedName] = substring(dfResult[i,]$NBASalaries.X2022.23,2)
#}
...
plot(x = dfResult$Salaries, y = dfResult$NBASalaries.Rk,
   xlab = "Salaries (millions)",
   ylab = "Rank",	 
   main = "Salaries vs Rank"
)

dfResult$RkPerformance = 30 - dfResult$NBASalaries.Rk
plot(x = dfResult$Salaries, y = dfResult$RkPerformance,
   xlab = "Salaries (millions)",
   ylab = "Rank",	 
   main = "Salaries vs Rank"
)
